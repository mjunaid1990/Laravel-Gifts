  ...
  <div class="card">
    <div class="card-body row">
      <div class="col-md-6 col-12">
        <form action="{{ route('menucategoryEditSave', ['category' => $category->slug, 'id' => $category->id]) }}" method="post" enctype="multipart/form-data">
          @csrf
          ...
          <div class="form-group">
            ...
            <input type="text" name="name" id="name" class="form-control" placeholder="Chicken" value="{{old('name')?old('name'):$category->name}}" required>
          </div>
          ...
          <div class="form-group">
            ...
            <select name="parent_category_id" id="parent_category_id" class="form-select">
              ...
              <option value="{{ $item->id }}" {{ $category->parent_category_id == $item->id ? 'selected' : '' }}>{{ str_replace('-', ' ', $item->name) }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select name="country" id="country" class="form-select" required>
              ...
              @foreach ($countries as $country)
                <option value="{{ $country->iso }}"  {{ $category->country == $country->iso ? 'selected' : '' }}>{{ $country->name }}</option>
              @endforeach
            </select>
          </div>
          
          <div class="form-check">
              <input class="form-check-input" name="is_show_menu" type="checkbox" value="1" id="is_show_menu" {{$category->is_show_menu == 1?'checked':''}}>
